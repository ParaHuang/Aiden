...
public class GridLayoutFrame extends JFrame {
	...
	private int rows=10;
	private JButton[] btns = new JButton[rows*rows];
	...
	public GridLayoutFrame() {
		...
		contentPane.setLayout(new GridLayout(rows, rows,2,2));
...
//			int index = i;
			...
			btns[i].addActionListener(new ActionListener() {
				...
				//event records every detail the it happens
				@Override
...
//					System.out.println(command);
					...
					//up
					if(btns[index-rows].getBackground() == Color.PINK) {
						btns[index-rows].setBackground(Color.BLUE);
					}else {
						btns[index-rows].setBackground(Color.PINK);
					}
					//down
					if(btns[index+rows].getBackground() == Color.PINK) {
						btns[index+rows].setBackground(Color.BLUE);
